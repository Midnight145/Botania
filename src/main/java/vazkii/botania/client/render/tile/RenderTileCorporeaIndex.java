...
public class RenderTileCorporeaIndex extends TileEntitySpecialRenderer {
	...
	public void renderTileEntityAt(TileEntity tileEntity, double x, double y, double z, float partialTicks) {
		TileCorporeaIndex index = (TileCorporeaIndex) tileEntity;

		GL11.glPushMatrix();
		GL11.glTranslated(x + 0.5, y, z + 0.5);
		Minecraft.getMinecraft().renderEngine.bindTexture(texture);

		float translation = move ? (float) ((Math.cos((index.ticksWithCloseby + (index.hasCloseby ? partialTicks : 0)) / 10F) * 0.5 + 0.5) * 0.25) : 0F;
		float rotation = move ? index.ticks * 2 + partialTicks : 0F;
		float scale = 0.6F;
