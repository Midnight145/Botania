...
public class RenderTileEnchanter extends TileEntitySpecialRenderer {
	...
	public void renderTileEntityAt(TileEntity tileEntity, double x, double y, double z, float partialTicks) {
		...
		float alpha = (float) ((Math.sin((ClientTickHandler.ticksInGame + partialTicks) / 8D) + 1D) / 5D + 0.4D) * alphaMod;
		...
		if(alpha > 0) {
			...
			RenderHelper.renderIcon(0, 0, BlockEnchanter.overlay, 5, 5, 240);
